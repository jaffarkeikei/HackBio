...
"""
Final Analysis Pipeline Script

This script fixes all the issues in the previous analysis scripts and
provides a complete solution for training and evaluating models.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("final_analysis.log"),
...
def train_models_final():
    """Train ensemble models with all issues fixed"""
    logger.info("Starting ensemble model training (final version)...")
    ...
    try:
        ...
        if os.path.exists('results/processed_data.parquet'):
            ...
            logger.error("No preprocessed data available. Please run data preprocessing first.")
            return False
        ...
        # Select top variable genes for faster training 
        ...
        if non_numeric_cols:
            ...
            for i, col in enumerate(non_numeric_cols):
                for cat in encoder.categories_[i]:
                    safe_cat = re.sub(r'[\[\]<>]', '_', str(cat))  # Replace problematic chars
                    encoded_col_names.append(f"{col}_{safe_cat}")
        ...
        # Ensure feature names are XGBoost compatible (no '[', ']', or '<')
        ...
        try:
            # Plot predictions vs actual for a few genes
            ...
            r2_values = [avg_rf_r2, avg_xgb_r2, avg_ensemble_r2]
            
            plt.bar(models, r2_values, color=['blue', 'green', 'red'])
            plt.ylabel('Average R² Score')
            plt.title('Model Performance Comparison')
            plt.ylim(0, 1.0)  # R² score ranges from 0 to 1
            ...
            plt.tight_layout()
            plt.savefig('plots/model_summary.png', dpi=300, bbox_inches='tight')
            plt.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run the final analysis pipeline")
    args = parser.parse_args()
    
    # Set up environment
    setup_environment()
    
    # Execute final training script
    success = train_models_final()
    ...
    if success:
        ...
        with open('results/analysis_summary.txt', 'w') as f:
            f.write("HackBio Pipeline Final Results\n")
            f.write("==============================\n\n")
            f.write("Pipeline completed successfully.\n\n")
            
            f.write("The following files were generated:\n")
            f.write("- Models: RF and XGBoost in models/ directory\n")
            f.write("- Performance metrics in results/ directory\n")
            f.write("- Visualizations in plots/ directory\n\n")
            
            f.write("Next steps:\n")
            f.write("1. Run correlation analysis\n")
            f.write("2. Complete model interpretation\n")
            f.write("3. Prepare final report\n")
        
    else:
        logger.error("Final analysis failed.")
